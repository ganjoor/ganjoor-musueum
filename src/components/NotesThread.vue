            ...
            <template v-slot:prepend="{ item }">
                <v-flex xs12>
                    <v-card :color="publicNoteColor(item)">
                        ...
                        <v-card-text>
                            <v-icon v-if="item.rUserImageId == null" size="60" class="centeredimg">account_circle</v-icon>
                            <v-img v-if="item.rUserImageId != null" max-width="60" max-height="60" class="centeredimg"
                                   :src="`${appConfig.$api_url}/api/rimages/${item.rUserImageId}.jpg`"></v-img>
                            <br />
                        ...
                        <v-card-actions>
                            ...
                            <v-spacer></v-spacer>
                            <v-tooltip v-if="userInfo != null && userInfo.user.id === item.rAppUserId" bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn text icon color="blue" v-on="on" v-on:click="editCurrentNote(item, 'public')">
                                        <v-icon>edit</v-icon>
                                    </v-btn>
                                </template>
                                <span>اصلاح</span>
                            ...
                            <v-layout justify-center v-if="editingPublicNote">
                                <v-dialog v-model="editingPublicNote"
                                          width="500">
                                    <v-card>
                                        ...
                                        <v-card-actions>
                                            ...
                                            <v-btn color="secondary" text @click="editingPublicNote = false">انصراف</v-btn>
                                        </v-card-actions>
                            ...
                            <!-- edit item end -->
                            <v-tooltip v-if="userInfo != null && (userInfo.user.id === item.rAppUserId || checkPermission('note', 'moderate') )" bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn text icon color="red" v-on="on" v-on:click="deleteNote(item)">
                            ...
                            <v-tooltip v-if="userInfo != null" bottom>
                                <template v-slot:activator="{ on }">
                                    <v-btn text icon color="green" v-on="on" v-on:click="replyingTo = item.id">
                                        <v-icon>reply</v-icon>
                                    </v-btn>
                                </template>
                        ...
                        <NotesThread v-if="item.notes.length != 0" :entity="entity" :notes="item.notes" :hashNoteId="hashNoteId" :itemId="itemId" :level="level + 1" :imageId="imageId" />
                            ...
                            <v-card>
                                <v-card-text>
                                    <v-textarea v-model="newNote"
                                                ...
                                                outlined
                                                rounded></v-textarea>
                                    <v-btn v-if="imageId != null" text v-on:click="insertCroppedImage = true">
                                        ...
                                        <v-dialog v-model="insertCroppedImage"
                                            ...
                                            <v-card>
                                                <cropper classname="cropper"
                                                         ...
                                                         @change="cropperCoordinatesChanged"></cropper>
...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                var postUrl = this.entity === 'artifact' ? this.appConfig.$api_url + "/api/artifacts/note" : this.appConfig.$api_url + "/api/artifacts/note/item";
                axios({                    
            ...
            onCropOKClicked() {
                ...
                axios({
                    method: "GET",
                    "url": this.appConfig.$api_url + "/api/images/cropped/" + this.imageId + "?left=" + this.coordinates.left + "&top=" + this.coordinates.top + "&width=" + this.coordinates.width + "&height=" + this.coordinates.height,
                        "data": {},
                        "headers":
                        {
                            "authorization": "bearer " + this.userInfo.token,
