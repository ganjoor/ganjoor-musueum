        ...
        <v-content class="pa-0">
            ...
            <v-layout justify-space-around v-if="userInfo === null && !verifying && !finalizing && !processing &!finalized" color="primary">
                <v-card>
                    ...
                    <v-card-text>
                        <v-btn color="error" v-on:click="forgot()">ادامه</v-btn>
                    </v-card-text>
                </v-card>
            </v-layout>
                ...
                <v-card>
                    <router-link to="/">
                        <v-btn color="green" class="ma-2 white--text" @click="signin">
                            تبریک! گذرواژهٔ شما تغییر کرد. برای ورود به سیستم کلیک کنید.        ‌<v-icon right dark>done</v-icon>
                        </v-btn>
                    </router-link>
                </v-card>
            </v-layout>
    ...
    export default {
        ...
        methods: {
            ...
            forgot() {
                ...
                this.processing = true;
                axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/users/forgotpassword", "data":
                    {
                        email: this.email,
            ...
            verify() {
                ...
                this.processing = true;
                axios({
                    method: "GET", "url": this.appConfig.$api_url + "/api/users/verify?type=1&secret=" + this.secret, "data": {},
                    "headers":
                    {
                        "content-type": "application/json"
                    }
            ...
            finalize() {
                ...
                this.processing = true;
                axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/users/resetpassword", "data":
                    {
                        email: this.email,
                        secret: this.secret,
