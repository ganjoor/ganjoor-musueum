    ...
    <div class="user">
        <v-content class="pa-0">            
            <v-layout justify-space-around v-if="userInfo == null" color="primary">
                <v-card>
                    <v-card-text>
                        <v-text-field prepend-inner-icon="person" v-model="input.username" placeholder="پست الکترونیکی" class="ltrinput"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                        <v-text-field prepend-inner-icon="lock" v-model="input.password" type="password" placeholder="گذرواژه" class="ltrinput"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                        <v-flex xs12 text-center>
                            <v-btn color="error" v-on:click="signin()">ورود</v-btn>
                        </v-flex>
                    </v-card-text>
                    <v-card-text>
                        <v-flex xs12 text-center>
                    ...
                    <router-link to="/profile">
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                ...
                                <v-img v-if="userInfo.user.rImageId != null" max-width="196" max-height="196" v-on="on" class="clickable centeredimg"
                                       :src="`${appConfig.$api_url}/api/rimages/${userInfo.user.rImageId}.jpg`"></v-img>
                            </template>
    ...
    export default {
        ...
        methods: {
            ...
            refreshNotificationsCount() {
                if (this.userInfo != null) {
                         ...
                         "headers":
                    {
                        ...
                        this.processing = false;
                            if (error.response && error.response.status == 401) {//probably session expired
                                this.signout();
                            }
                            else {
            ...
            signin() {
                this.processing = true;
                this.errMsg = '';
                axios({ method: "POST", "url": this.appConfig.$api_url + "/api/users/login", "data": this.input, "headers": { "content-type": "application/json" } }).then(result => {                  
            ...
            signout() {
                axios({
                    method: "DELETE", "url": this.appConfig.$api_url + "/api/users/delsession?userId="+this.userInfo.user.id+"&sessionId="+this.userInfo.sessionId,
                    "headers":
                    {
                        "content-type": "application/json",
                        "authorization": "bearer " + this.userInfo.token
