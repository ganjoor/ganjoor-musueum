        ...
        <v-layout wrap>
            <v-flex xs12 v-if="userInfo != null">
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-text-field prepend-inner-icon="lock" v-model="oldPassword" type="password" placeholder="گذرواژه فعلی" class="ltrinput"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                        <v-text-field prepend-inner-icon="lock" v-model="newPassword" type="password" placeholder="گذرواژه جدید" class="ltrinput"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                        <v-text-field prepend-inner-icon="lock" v-model="confirmPassword" type="password" placeholder="تکرار گذرواژه جدید" class="ltrinput"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                ...
                <v-card>
                    ...
                    <v-card-text>
                        <v-text-field prepend-inner-icon="person" v-model="userInfo.user.firstName" placeholder="نام"></v-text-field>
                    </v-card-text>
                    <v-card-text>
                        <v-text-field prepend-inner-icon="person" v-model="userInfo.user.sureName" placeholder="نام خانوادگی"></v-text-field>
                    </v-card-text>
                    <v-card-text>
    ...
    export default {
        ...
        methods: {            
            ...
            _handleImageUpload() {
                ...
                axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/users/image", "data": data
                    ,
                    "headers":
                    {
            ...
            changePassword() {
                ...
                axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/users/setmypassword", "data": { "oldPassword": this.oldPassword, "newPassword": this.newPassword }
                    ,
                    "headers":
                    {
            ...
            updateUser() {
                this.errorMsg = '';
                axios({
                        method: "PUT", "url": this.appConfig.$api_url + "/api/users/" + this.userInfo.user.id,
                        "data":
                        {
                            "password": "",
