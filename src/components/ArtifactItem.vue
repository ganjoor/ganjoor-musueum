        ...
        <v-card light v-if="item != null">
          ...
          <v-card dark>
            <v-card-text v-if="item != null">
              تصویر {{ item.item.order }} از {{ item.parentItemCount }}
              <router-link :to="`/items/${item.parentFriendlyUrl}`">{{
                item.parentName
              }}</router-link>
              <a :href="`${getViewerUrl()}`">
        ...
        <v-layout wrap v-if="item != null">
          ...
          <v-flex xs12>
            <v-card>
              <v-card-text>
                <router-link
                  :to="`/items/${item.parentFriendlyUrl}/pageno/${
                    Math.floor((item.item.order - 1) / 21) + 1
                  }`"
                >
      ...
      <v-flex xs8>
        <div v-if="item != null">
          <v-tabs v-model="active_tab">
            <v-tab :href="`#tab-1`"> تصویر </v-tab>
            <v-tab :href="`#tab-2`"> ویژگیها </v-tab>
            <v-tab v-if="userInfo != null" :href="`#tab-3`">
              یادداشتهای شخصی
            </v-tab>
            <v-tab :href="`#tab-4`"> یادداشتهای عمومی </v-tab>
          </v-tabs>
              ...
              <v-flex xs12>
                <v-card dark color="secondary">
                  ...
                  <v-tooltip
                    bottom
                    v-if="
                      imageAngle != 0 && checkPermission('artifact', 'modify')
                    "
                  >
                    <template v-slot:activator="{ on }">
                      <v-btn
    ...
    >
      <v-card>
        <v-toolbar dark color="primary">
          ...
          <v-toolbar-items>
            <v-btn dark color="green" @click="saveGanjoorLinkSuggestion()"
              >پیشنهاد<v-icon left>done</v-icon></v-btn
            >
          </v-toolbar-items>
        </v-toolbar>
        <v-flex xs12>
          <v-card>
            <v-card-text>
              در قاب سمت راست شعر مرتبط در گنجور را پیدا و مشخص کنید و در قاب
              سمت چپ تصویر مورد نظر را و نهایتاً روی دکمهٔ «<span color="green"
                ><strong>پیشنهاد</strong></span
              >» کلیک کنید. می‌توانید بدون بستن پنجره این کار را برای شعرها و
              تصاویر بعدی تکرار کنید.
            </v-card-text>
          </v-card>
        </v-flex>
        <v-layout row wrap>
          ...
          <v-flex xs6>
            <iframe
              v-if="item != null"
              :src="`/items/${item.parentFriendlyUrl}/${item.item.friendlyUrl}`"
              style="width: 50vw; height: 70vh"
            ></iframe>
          </v-flex>
        </v-layout>
  ...
  methods: {
    ...
    processItemResult(result) {
      ...
      document.title =
        ...
        " - تصویر " +
        this.item.item.order +
        " از " +
        this.item.parentItemCount;
        ...
        axios({
          ...
          (bookmarks) => {
            ...
            axios({
              ...
              url:
                this.appConfig.$api_url +
                "/api/artifacts/note/item/private/" +
                this.item.item.id,
              data: {},
              headers: {
                authorization: "bearer " + this.userInfo.token,
    ...
    saveNewNote(noteType) {
      ...
      axios({
        method: "POST",
        url: this.appConfig.$api_url + "/api/artifacts/note/item",
        data: {
    ...
    saveGanjoorLinkSuggestion() {
      var ganjoorUrl = strReplacer.replaceAll(
        strReplacer.replaceAll(
          this.$refs.ganjoorFrame.contentWindow.location.href.substring(
            this.$refs.ganjoorFrame.contentWindow.location.href.indexOf("=") + 1
          ),
          "%3A",
          ":"
        ),
      ...
      axios({
        method: "POST",
        url: this.appConfig.$api_url + "/api/artifacts/ganjoor",
        data: {
          ganjoorPostId,
          ganjoorUrl,
          ganjoorTitle,
        ...
        (result) => {
          ...
          this.suggestionResult =
            result.data.ganjoorTitle + " <==> " + result.data.entityName;
        },
        (error) => {
          this.longProcessInDialogs = false;
    ...
    cover() {
      ...
      axios({
        ...
        url:
          this.appConfig.$api_url +
          "/api/artifacts/" +
          this.item.item.rArtifactMasterRecordId +
          "/cover",
        data: this.item.item.order - 1,
        headers: {
          authorization: "bearer " + this.userInfo.token,
          "content-type": "application/json",
        },
        ...
        if (confirm("از ذخیرهٔ چرخش فعلی اطمینان دارید؟")) {
          ...
          axios({
            ...
            url:
              this.appConfig.$api_url +
              "/api/images/" +
              this.item.item.images[0].id +
              "/" +
              this.imageAngle.toString(),
            data: {},
            headers: {
              authorization: "bearer " + this.userInfo.token,
              "content-type": "application/json",
