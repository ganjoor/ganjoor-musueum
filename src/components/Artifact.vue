            ...
            <v-flex xs4>
                <v-card dark color="secondary">
                    <v-card-text v-if="item != null">
                        ...
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on }">
                                <v-chip outlined color="white" v-on="on" v-on:click="showCoverImage()"><v-icon dark>image</v-icon></v-chip>
                            </template>
                    ...
                    <v-card dark color="primary">
                        ...
                        <v-card-text v-if="item != null && filteredTag != null">تصاویر با ویژگی {{filteredTag.name}} <span v-if="filteredTagValue != null"> دارای مقدار {{filteredTagValue.value}}</span>&nbsp;<small>({{item.items.length}} تصویر)</small></v-card-text>
                    </v-card>
                    <!-- edit name begin -->
                    <v-layout justify-center v-if="item != null && checkPermission('artifact', 'modify')">
                        <v-dialog v-model="editingName"
                                  width="500">
                            <template v-slot:activator="{ on }">
                            ...
                            <v-card>
                                ...
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn color="primary" @click="onEditNameOKClicked()">تأیید</v-btn>
                                    <v-btn color="secondary" text @click="editingName = false">انصراف</v-btn>
                                </v-card-actions>
                    ...
                    <v-tooltip  v-if="item != null && checkPermission('artifact', 'delete') && item.status != 4" bottom>
                        <template v-slot:activator="{ on }">
                            <v-btn icon text color="white" v-on="on" v-on:click="deleteArtifact()"><v-icon dark>delete</v-icon></v-btn>
                        </template>
                        <span>حذف</span>
                    </v-tooltip>                   
                </v-card>
            ...
            <v-flex xs8>
                <v-card>
                    <v-tabs v-if="item != null" v-model="active_tab">
                        ...
                        <v-tab v-if="item.contents.length > 0" :href="`#tab-3`">فهرست</v-tab>
                        <v-tab v-if="userInfo != null" :href="`#tab-4`">یادداشتهای شخصی</v-tab>
                    ...
                    <v-tabs-items v-if="item != null" v-model="active_tab">
                        <v-tab-item :value="`tab-1`">
                            ...
                            <v-card dark color="secondary">
                                ...
                                <v-layout justify-center v-if="checkPermission('artifact', 'modify')">
                                    <v-dialog v-model="editingDescription"
                                              width="500">
                                        <template v-slot:activator="{ on }">
                                            <v-btn text icon color="white" dark v-on="on">
                                                <v-icon>edit</v-icon>
                                        ...
                                        <v-card>
                                            <v-card-text>
                                                <v-textarea label="توضیحات" auto-grow rows="20" v-model="itemDescription"></v-textarea>
                                            </v-card-text>
                                            ...
                                            <v-card-actions>
                                                <v-spacer></v-spacer>
                                                <v-btn color="primary" @click="onEditDescriptionOKClicked()">تأیید</v-btn>
                                                <v-btn color="secondary" text @click="editingDescription = false">انصراف</v-btn>
                                            </v-card-actions>
                        ...
                        <v-tab-item :value="`tab-2`">
                            <v-card v-if="checkPermission('tag', 'modify') && item.artifactTags.length == 0">
                                <v-card-text>
                            ...
                            <v-simple-table>
                                <tbody>
                                    <tr v-for="tag in item.artifactTags" :key="tag.id">
                                        <td v-if="tag.friendlyUrl != null && (tag.tagType == 2 || tag.tagType == 3)">
                                ...
                                <tbody>
                                    <tr v-for="value in tag.values" :key="value.id">
                                        <td v-if="tag.friendlyUrl != null && value.friendlyUrl != null && (tag.tagType == 2 || tag.tagType == 3)">
                                            <router-link :to="`/tags/${tag.friendlyUrl}/${value.friendlyUrl}`">
                                                {{value.value}}
                                            </router-link>
                                        </td>
                                        <td v-if="((tag.tagType == 2 || tag.tagType == 3) && (tag.friendlyUrl == null || value.friendlyUrl == null)) || (tag.tagType != 2 && tag.tagType != 3 && tag.tagType != 4)" v-html="value.value"></td>
                            ...
                            <tr v-for="rTagSum in item.rTagSums" :key="rTagSum.tagFriendlyUrl">
                                <td>
                                    <router-link :to="`/items/${item.friendlyUrl}/tags/${rTagSum.tagFriendlyUrl}`">
                                        {{rTagSum.tagName}}
                                    </router-link>
                            ...
                            <v-layout justify-center v-if="checkPermission('artifact', 'edittag')">
                                <v-dialog v-model="editingTagValue"
                                    ...
                                    <v-card>
                                        ...
                                        <v-card-text>
                                            <v-text-field label="پیوند" v-model="tagValueLink"></v-text-field>
                                        </v-card-text>
                                    ...
                                    <tr v-for="content in item.contents" :key="content.order">
                                        <td>
                                            <p :style="`font-size:1.${content.level}em`">
                                                <router-link :to="`/items/${item.friendlyUrl}/${content.itemFriendlyUrl}`">
                                                    {{content.title}}
                                                </router-link>
                                            </p>
                                        </td>
                        ...
                        <v-tab-item :value="`tab-5`">
                            ...
                            <NotesThread v-if="publicnotes != null" :entity="`artifact`" :notes="publicnotes" :hashNoteId="hashNoteId" :itemId="item.id" :level="0" />
                        </v-tab-item>
                    </v-tabs-items>
                ...
                <v-card>
                    <v-container v-if="pageItems != null" grid-list-sm fluid>
                        <v-layout row wrap>
                            <v-flex v-for="artifactItem in pageItems"
                                    :key="artifactItem.id"
                                    d-flex>
                                <v-card flat tile class="d-flex">

                                    <router-link :to="`/items/${item.friendlyUrl}/${artifactItem.friendlyUrl}`">
                                        <v-img :src="`${appConfig.$api_url}/api/images/thumb/${artifactItem.images[0].id}.jpg`"
                                               max-width="200px"
                                               max-height="400px"
                                            ...
                                            <v-chip outlined color="white">{{artifactItem.order}}<v-icon dark>remove_red_eye</v-icon></v-chip>
                                        </v-img>
                                    </router-link>
        ...
        methods: {            
            loadArtifact() {
                ...
                if (this.userInfo == null) {
                    apiUrl = this.appConfig.$api_url + "/api/artifacts/" + this.$route.params.friendlyUrl;
                    ...
                    apiUrl = this.appConfig.$api_url + "/api/artifacts/secure/" + this.$route.params.friendlyUrl;
                    if (this.$route.params.tag != null) {
                        apiUrl += ('/filteritemsbytag/' + this.$route.params.tag);
                        if (this.$route.params.value != null) {
                            apiUrl += ('/' + this.$route.params.value);
            ...
            processArtifactResult(result) {               
                ...
                var selectedItems = [];
                if (this.$route.params.tag != null) {//no paging
                    ...
                    for (var i = (this.pageNumber - 1) * this.pageSize; i < Math.min(this.pageNumber * this.pageSize, this.item.items.length); i++) {
                        selectedItems.push(this.item.items[i]);
                    }
                    this.pageItems = selectedItems;
                    this.pageCount = (this.item.items.length % this.pageSize) == 0 ? parseInt(this.item.items.length / this.pageSize, 10) : parseInt(this.item.items.length / this.pageSize, 10) + 1; 
                }
                ...
                this.itemDescription =
                    strReplacer.replaceArray(this.item.description, ['<br>', '<br/>', '<br />'], '\r\n');                   

                this.errorMsg = '';

               this.loadingPublicNotes = true;
                axios({
                    method: "GET", "url": this.appConfig.$api_url + "/api/artifacts/note/" + this.item.id,
                    "data": {},
                    "headers":
                    {
                        "content-type": "application/json"
                ...
                if (this.userInfo != null) {
                    ...
                    axios({
                        method: "GET", "url": this.appConfig.$api_url + "/api/artifacts/bookmark/" + this.item.id,
                        "data": {},
                        "headers":
                        {
                            "authorization": "bearer " + this.userInfo.token,
                        ...
                        axios({
                            method: "GET", "url": this.appConfig.$api_url + "/api/artifacts/note/private/" + this.item.id,
                            "data": {},
                            "headers":
                            {
                                "authorization": "bearer " + this.userInfo.token,
                ...
                if (this.$route.params.tag != null) {
                    ...
                    axios({
                        method: "GET", "url": this.appConfig.$api_url + "/api/artifacts/tag/" + this.$route.params.tag, "data": {},
                        "headers":
                        {
                            "content-type": "application/json"
                        }
                        ...
                        if (this.$route.params.value != null) {
                            this.errorMsg = '';
                            axios({
                                method: "GET", "url": this.appConfig.$api_url + "/api/artifacts/tagvalue/" + this.filteredTag.id + '/' + this.$route.params.value, "data": {},
                                "headers":
                                {
                                    "content-type": "application/json"
                                }
            ...
            pageNumberChanged(pageNumber) {
                ...
                this.$router.push('/items/' + this.item.friendlyUrl + '/pageno/' + pageNumber);               
            ...
            onEditNameOKClicked() {
                ...
                axios({
                    method: "PUT", "url": this.appConfig.$api_url + "/api/artifacts/" + this.item.id, "data": this.item,
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            addTagValue(rTag) {
                this.longProcessInDialogs = true;
                axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/artifacts/tagvalue/" + this.item.id, "data": rTag,
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            moveTag(tag, upOrDown) {
                this.errorMsg = '';
                axios({
                    method: "PUT", "url": this.appConfig.$api_url + "/api/artifacts/tag/move/" + tag.id + '/in/' + this.item.id + '/' + upOrDown , "data": {},
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            removeTagValue(value) {
                if (confirm("از حذف این مقدار ویژگی اطمینان دارید؟")) {
                    this.errorMsg = '';
                    axios({
                        method: "DELETE", "url": this.appConfig.$api_url + "/api/artifacts/tagvalue/" + this.item.id + '/' + value.id, "data": {},
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            moveValue(value, upOrDown) {
                this.errorMsg = '';
                axios({
                    method: "PUT", "url": this.appConfig.$api_url + "/api/artifacts/tagvalue/" + this.item.id + '/' + value.rTagId + '/' + value.id + '/move/' + upOrDown, "data": {},
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            onEditTagValueOKClicked() {
                ...
                this.longProcessInDialogs = true;
                axios({
                    method: "PUT", "url": this.appConfig.$api_url + "/api/artifacts/tagvalue/" + this.item.id + '/' + this.tagValueEditGlobally.toString(), "data": this.tagValueObject,
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            getFormattedValue(rTagValue, rTag) {
                switch (rTag.tagType) {
                    case 2: //Search
                    case 3: //SearchLink
                        return '<a href="/tags/' + rTag.friendlyUrl + '/' + rTagValue.friendlyUrl + '">' + rTagValue.value + '</a>';
                }
            ...
            bookmark() {
                ...
                if (this.bookmarked) {
                    ...
                    axios({
                    method: "POST", "url": this.appConfig.$api_url + "/api/artifacts/bookmark/" + this.item.id , "data": {},
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
            ...
            saveNewNote(noteType) {
                ...
                axios({                    
                    method: "POST", "url": this.appConfig.$api_url + "/api/artifacts/note",
                    "data":
                    {
            ...
            showCoverImage() {
                if (this.item != null) {
                    this.$router.push('/items/' + this.item.friendlyUrl + '/' + this.item.items[this.item.coverItemIndex].friendlyUrl);
                }
            },
            deleteArtifact() {
                if (confirm("از حذف این شیء اطمینان دارید؟")) {
                    this.errorMsg = '';
                    axios({
                        method: "DELETE", "url": this.appConfig.$api_url + "/api/artifacts/" + this.item.id, "data": {},
                    "headers":
                    {
                        "authorization": "bearer " + this.userInfo.token,
                        "content-type": "application/json"
